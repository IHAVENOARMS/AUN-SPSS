...
varfmtsrev = dict([(v, k) for k, v in varfmts.items()])
...
def _getIndexInfo(indexes, datelist):
    ...
    if not indexes or indexes[0] == "ALL":
        indexes = list(range(spss.GetVariableCount()))
    numrequestedvars = len(indexes)

    if isinstance(indexes[0], str):      #list of names
    ...
    if datelist:
        indexes = list(filter(spssaux.isDateVariable, indexes))  # restrict to date variables
