...
def unicodeit(value, myenc):
    ...
    if value is None:
        return ""
    if not isinstance(value, str):  # could be str or a class
        try:
            return  str(value, myenc)
        except:
            return str(str(value), myenc)
    else:
        return value
...
def transupport(thefile,localedir=None, lang=None, private=False):
    ...
    if private:
        ...
        gettext.install(thename, localedir=localedir)
        if origlang:
            os.environ["LANGUAGE"] = origlang
...
def helper(helpfile="markdown.html"):
    ...
    helpspec = os.path.join("file:///", helpspecbase)
    if sys.platform.lower().find('darwin') > -1:
        helpspec = "file:///" + helpspec
