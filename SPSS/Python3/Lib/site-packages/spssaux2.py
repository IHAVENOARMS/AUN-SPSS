...
ok1600 = int(spssaux.getSpssVersion().split(".")[0]) >= 16
ok1800 = int(spssaux.getSpssVersion().split(".")[0]) >= 18
...
def FindMostRecentFile(basename):
    ...
    candidates = \
        list(filter(os.path.isfile, candidates\
         + list(filter(os.path.exists, [basename, root+ext]))))        
    if not candidates:
        return None
    # find most recent
    candidates = list(zip(list(map(os.path.getmtime, candidates)),candidates))
...
AGGREGATE OUTFILE=%(outputds)s""" % locals()]
    ...
    if vars is None:  # check all variables (except as governed by alpha)
        stillstanding = set(range(spss.GetVariableCount()))
    else:
...
def FindUnlabelledValues(vardict):
        ...
        with spss.DataStep():
            ...
            for v in vardict:
                ...
                if len(lbls) > 0:
                    ...
                    if v.VariableType > 0:
                        lbls = [k.rstrip() for k in list(lbls.keys())]
                    else:
                        lbls = [k for k in list(lbls.keys())]
    ...
    unlabelled = {}
    for i in range(len(varstocheck)):
...
def rankvarsincase(varlist, suffix="_rank"):
    ...
    indexes = list(range(len(varlist)))
...
def mergeByLabel(firstds, secondds):
    ...
    renamein = []
    renameout = []
    for (label, name) in list(labeldict1.items()):
...
"""
    ...
    for f in otherfuncs:
        if not isinstance(f, collections.Callable):
            raise ValueError("A value for otherfuncs is not a callable function: %s" % f.__name__)
    ...
    curs.append("count")
    for f in otherfuncs:
        curs.append(f.__name__)
    curs.commitdict()
    for date, count in list(datecounts.items()):
        curs.appendvalue("tallydate", date)
        curs.appendvalue("count", count)
        if numfuncs:
            fvalues = funcoutputs[date]
            for i, f in enumerate(otherfuncs):
                curs.appendvalue(f.__name__, fvalues[i])
        curs.CommitCase()
    curs.CClose()
