...
def getAttributesDict(varname=None):
    ...
    spss.DeleteXPathHandle(tag)    
    return dict(list(zip(attnames, attvalues)))
    ...
    try:
        ...
        for i,v in enumerate(values):
            if v == "<none>":
                values[i] = None
        spss.DeleteXPathHandle(tag[0])
        result = dict(list(zip(items, values)))
...
def    getValueLabels(varindex, matchtype=False):
    ...
    return dict(list(zip(vallist, lablist)))
...
def openDataFile(filespec, datasetName=None, dataset=None, varlist=None, filetype='sav', **kwargs):
    ...
    if filetype == "sav":
        ...
        if varlist.strip():
            varlist = " /KEEP=" + (isinstance(varlist, str) and varlist or ' '.join(varlist))
...
def openDataFileFromUrl(url,  *args, **kwds):
    ...
    unqname = urllib.parse.unquote_plus(url)
...
"""
    ...
    def _ValLabSet(self, vldict):
        ...
        vllist = " ".join([_usmartquote(k) + " " + _usmartquote(v) for k, v in list(vldict.items())])
    ...
    def _AttrSet(self, attrdict):
        ...
        attrlist = " ".join([k + "(" + _smartquote(str(v))+ ")" for k, v in list(attrdict.items())])
...
class CasingDict(dict):
    ...
    def keys(self):
        ...
        if self.caseless:
            return list(self.origcase.values())
        else:
    ...
    def iterkeys(self):
        if self.caseless:
            return iter(self.origcase.values())
        else:
...
class VariableDict(object):
    ...
    def __init__(self, namelist=None, safe=False, pattern=None, variableType=None, variableLevel=None, 
        ...
        elif isinstance(namelist[0], str):        #type(namelist[0]) is str:    # got list of names
    ...
    def variablesf(self, variableType=None, variableLevel=None, pattern=None):
        ...
        if match.noop:
            ...
            return [var for var in list(self.vdict.keys()) if match(self.vdict[var], var)]
    ...
    def expand(self, vlist):
        ...
        if not hasattr(self, "simplelist"):
            self.simplelist = [v.lower() for (v, l) in sorted(list(self.vdict.items()), key=operator.itemgetter(1))]
        resultlist = []
        try:
    ...
    def indexes(self, variableType=None, variableLevel=None, pattern=None):
        ...
        match = makeVariableFilter(pattern, variableType, variableLevel)
        if match.noop:
            return list(self.vdict.values())
        else:
            return [self.vdict[var] for var in list(self.vdict.keys()) if match(self.vdict[var], var)]
    ...
    def variableIndex(self, id):
        ...
        if isinstance(id, str):            #type(id) is str:
            res = self.vdict[id]
    ...
    def variableNamesFromLabel(self, label):
        ...
        """
        return [name for (name, index) in list(self.vdict.items()) if spss.GetVariableLabel(index) == label]
